...
Mat DetectLane::preProcess(const Mat &src)
{
    ...
    // HoughCircles(imgThresholded, circles, CV_HOUGH_GRADIENT, 1, 20, 30, 15, 10, 100);
...
/**
 * Split the screen to layers
 * Return result is a vector of 2D matrix
 * 
 * VERTICAL:
 * ..................................
 * ..................................
...
{
    int rowN = src.rows; // number of horizontal pixels
    int colN = src.cols; // number of vertical pixels
    std::vector<Mat> res;

    if (dir == VERTICAL)
    {
        for (int i = 0; i < rowN - slideThickness; i += slideThickness) { // slideThickness: step to slide
            Mat tmp;
            Rect crop(0, i, colN, slideThickness); // create a rectangle crop with a pair of (x, y, width, height)
            tmp = src(crop); // create a Mat instance from the crop
            res.push_back(tmp);
        }
    }
    else 
...
vector<vector<Point> > DetectLane::centerRoadSide(const vector<Mat> &src, int dir)
{
    ...
    int inputN = src.size(); // number of matrices
    for (int i = 0; i < inputN; i++) {
        std::vector<std::vector<Point> > cnts;
        std::vector<Point> tmp;
        findContours(src[i], cnts, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE, Point(0, 0)); // find matrix's contour
        int cntsN = cnts.size();
        if (cntsN == 0) {
            res.push_back(tmp);
            continue;
